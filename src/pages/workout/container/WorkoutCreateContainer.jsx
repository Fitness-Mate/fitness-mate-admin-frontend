...
class WorkoutCreateContainer extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    return (
      ...
      this.props.description !== nextProps.description ||
			this.props.atcetera !== nextProps.atcetera ||
  ...
  setConfirmMethod(method) {
    if (method === "create")
      this.setState({ confirmMethod: this.ajaxCreateWorkout });
  }
  ...
  async handleInputChange(e) {
    ...
    if (e.target.name === "korName")
      await WorkoutCreateAction.setKorName(e.target.value);
    if (e.target.name === "engName")
      await WorkoutCreateAction.setEngName(e.target.value);
    if (e.target.name === "image")
      await WorkoutCreateAction.setImage(e.target.files[0]);
    if (e.target.name === "videoLink")
      await WorkoutCreateAction.setVideoLink(e.target.value);
    if (e.target.name === "description")
      await WorkoutCreateAction.setDescription(e.target.value);
		if (e.target.name === "atcetera")
      await WorkoutCreateAction.setAtcetera(e.target.value);
  }
  ...
  ajaxCreateWorkout() {
    ...
    data.append("description", this.props.description);
		data.append("atcetera", this.props.atcetera);
    data.append("image", this.props.image);
    data.append(
      "machineIdList",
      this.props.selectedMachineList.map((machine) => machine.id)
    );
    data.append(
      "bodyPartIdList",
      this.props.selectedBodyPartList.map((bodyPart) => bodyPart.id)
    );
  ...
  render() {
    const { confirmMethod, confirmMessage, alertRedirectUrl, alertMessage } =
      this.state;
    ...
    const { korName, engName, description, atcetera, videoLink } = this.props;
              ...
              <td>
                {selectedMachineList.map((item, index) => (
                  <button
                    key={index}
                    onClick={() => {
                      this.handleMachineRemove(JSON.stringify(item));
                    }}
                  >
                    {item.koreanName}
                  </button>
                ))}
                <span style={{ fontSize: 10, color: "#FF0000" }}>
                  {" "}
                  *버튼을 누르면 제거
                </span>
              </td>
            ...
            <tr>
              ...
              <td>
                <select
                  ...
                  onChange={this.handleBodyPartSelect}
                >
              ...
              <td>
                {selectedBodyPartList.map((item, index) => (
                  <button
                    key={index}
                    onClick={() => {
                      this.handleBodyPartRemove(JSON.stringify(item));
                    }}
                  >
                    {item.koreanName}
                  </button>
...
export default connect(
  (state) => ({
    ...
    description: state.workoutCreate.description,
		atcetera: state.workoutCreate.atcetera,
